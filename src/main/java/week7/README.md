...
* 클래스패스 : 클래스를 찾기 위한 경로(JVM이 프로그램을 실행할 때, 클래스 파일을 찾는 데에 기준이 되는 경로)
* 설정하지 않으면 디폴트는 현재경로.
...
* 윈도우의 경우 환경변수 설정으로 변수이름 CLASSPATH로 설정하면 전역적으로 설정한다.
...
```
$java -classpath "classpath 경로" 실행파일명
```

* "경로;경로" => 윈도우는 ;로 여러개의 경로를 적을 수 있고, 유닉스 리눅스는 :를 사용한다
*  java -classpath ".;lib" 파일명
* 우선 .으로 현재폴더에서 class를 찾고, 없으면 lib에서 class를 찾는다 라는 뜻
